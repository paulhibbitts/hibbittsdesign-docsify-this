...
If nesting content files is required, use explicit relative path prefixes for all links — `./filename.md` for same-directory links and `../` or `../../` for parent directories (e.g. `../readme.md`, `../../readme.md`). For parent directory links, also add the URL parameter `?relative-paths=true` to your Docsify-This URL. Note that this parameter is not compatible with full root-relative paths (e.g. `level1/foo.md`).
